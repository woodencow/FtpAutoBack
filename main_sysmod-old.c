...
static void get_latest_webdav_timestamp_and_sequence(const char* username, const char* folder_name, char* latest_timestamp, size_t timestamp_size, int* sequence_num);
...
static Result stream_zip_to_webdav(const char* local_zip_path, u64 tid, AccountUid user_uid, const char* username) {
    ...
    if (curl_encode) {
        ...
        if (enc_title) strncpy(encoded_title, enc_title, sizeof(encoded_title) - 1);
        else strncpy(encoded_title, sanitized_title, sizeof(encoded_title) - 1);
        
        if (enc_username) strncpy(encoded_username, enc_username, sizeof(encoded_username) - 1);
        else strncpy(encoded_username, sanitized_username, sizeof(encoded_username) - 1);
        
        if (enc_ntp_timestamp) strncpy(encoded_ntp_timestamp, enc_ntp_timestamp, sizeof(encoded_ntp_timestamp) - 1);
        else strncpy(encoded_ntp_timestamp, ntp_timestamp, sizeof(encoded_ntp_timestamp) - 1);
    ...
    // 构造JKSV格式的WebDAV上传URL：JKSV/{标题安全路径}/{用户安全昵称}-{日期字符串}.zip
    if (webdav_config.basepath[0] != '\0') {
        // 确保origin和basepath之间有正确的斜杠分隔符
        if (webdav_config.origin[strlen(webdav_config.origin)-1] == '/' && webdav_config.basepath[0] == '/') {
            // origin以/结尾，basepath以/开头，移除basepath开头的/
            snprintf(url, sizeof(url), "%s%sJKSV/%s/%s_%s_%s.zip", 
                     webdav_config.origin, webdav_config.basepath + 1, encoded_title, encoded_title, encoded_username, encoded_ntp_timestamp);
        } else if (webdav_config.origin[strlen(webdav_config.origin)-1] != '/' && webdav_config.basepath[0] != '/') {
            // origin不以/结尾，basepath不以/开头，添加/
            snprintf(url, sizeof(url), "%s/%s/JKSV/%s/%s_%s_%s.zip", 
                     webdav_config.origin, webdav_config.basepath, encoded_title, encoded_title, encoded_username, encoded_ntp_timestamp);
        } else {
            // 其中一个有/，直接拼接
            snprintf(url, sizeof(url), "%s%sJKSV/%s/%s_%s_%s.zip", 
                     webdav_config.origin, webdav_config.basepath, encoded_title, encoded_title, encoded_username, encoded_ntp_timestamp);
        ...
        // 确保origin以/结尾
        if (webdav_config.origin[strlen(webdav_config.origin)-1] == '/') {
            snprintf(url, sizeof(url), "%sJKSV/%s/%s_%s_%s.zip", 
                     webdav_config.origin, encoded_title, encoded_title, encoded_username, encoded_ntp_timestamp);
        } else {
            snprintf(url, sizeof(url), "%s/JKSV/%s/%s_%s_%s.zip", 
                     webdav_config.origin, encoded_title, encoded_title, encoded_username, encoded_ntp_timestamp);
        }
    }
    ...
    // 创建JKSV格式的WebDAV目录结构：JKSV/{标题安全路径}
    char jksv_base_dir[256];
    char jksv_game_dir[256];
    ...
    if (webdav_config.basepath[0] != '\0') {
        ...
        if (webdav_config.origin[strlen(webdav_config.origin)-1] == '/' && webdav_config.basepath[0] == '/') {
            // origin以/结尾，basepath以/开头，移除basepath开头的/
            snprintf(jksv_base_dir, sizeof(jksv_base_dir), "JKSV");
        } else if (webdav_config.origin[strlen(webdav_config.origin)-1] != '/' && webdav_config.basepath[0] != '/') {
            // origin不以/结尾，basepath不以/开头，添加/
            snprintf(jksv_base_dir, sizeof(jksv_base_dir), "JKSV");
        } else {
    ...
    // 创建JKSV基础目录
    if (!create_webdav_directory(jksv_base_dir)) {
        snprintf(debug_buf, sizeof(debug_buf), "Failed to create WebDAV directory: %s", jksv_base_dir);
        log_file_write(debug_buf);
        // 不返回错误，继续尝试上传
    } else {
        snprintf(debug_buf, sizeof(debug_buf), "WebDAV directory created or already exists: %s", jksv_base_dir);
        log_file_write(debug_buf);
    }
    
    // 创建游戏标题目录
    snprintf(jksv_game_dir, sizeof(jksv_game_dir), "JKSV/%s", sanitized_title);
    if (!create_webdav_directory(jksv_game_dir)) {
        snprintf(debug_buf, sizeof(debug_buf), "Failed to create WebDAV directory: %s", jksv_game_dir);
        log_file_write(debug_buf);
        // 不返回错误，继续尝试上传
    } else {
        snprintf(debug_buf, sizeof(debug_buf), "WebDAV directory created or already exists: %s", jksv_game_dir);
        log_file_write(debug_buf);
    }
...
// 修改生成存档的函数，使用流式传输
// WebDAV握手函数 - 使用标准socket接口，握手成功后直接进行流式上传
        ...
        if (R_SUCCEEDED(dir_rc)) {
            ...
            if (entry_count > 0) {
                ...
                if (R_SUCCEEDED(rc)) {
                    ...
                    snprintf(final_path, sizeof(final_path), "/autoback/%s/%s/%s_%s_%s_%d.zip", 
...
// Global variables for tracking socket status
...
// Initialize BSD sockets (for FTP service use)
static Result initialize_bsd_sockets(void) {
...
// Initialize standard sockets (for WebDAV use)
static Result initialize_standard_sockets(void) {
...
static void manage_webdav_backup_count(const char* username, const char* game_folder) {
    ...
    if (webdav_config.basepath[0] != '\0') {
        if (webdav_config.origin[strlen(webdav_config.origin)-1] == '/' && webdav_config.basepath[0] == '/') {
            snprintf(url, sizeof(url), "%s%sJKSV/%s/", 
                     webdav_config.origin, webdav_config.basepath + 1, encoded_game_folder);
        } else if (webdav_config.origin[strlen(webdav_config.origin)-1] != '/' && webdav_config.basepath[0] != '/') {
            snprintf(url, sizeof(url), "%s/%s/JKSV/%s/", 
                     webdav_config.origin, webdav_config.basepath, encoded_game_folder);
        } else {
            snprintf(url, sizeof(url), "%s%sJKSV/%s/", 
                     webdav_config.origin, webdav_config.basepath, encoded_game_folder);
        }
        ...
        if (webdav_config.origin[strlen(webdav_config.origin)-1] == '/') {
            snprintf(url, sizeof(url), "%sJKSV/%s/", webdav_config.origin, encoded_game_folder);
        } else {
            snprintf(url, sizeof(url), "%s/JKSV/%s/", webdav_config.origin, encoded_game_folder);
        }
    }
    
    snprintf(debug_buf, sizeof(debug_buf), "Managing WebDAV backup count for user: %s, game: %s", username, game_folder);
    log_file_write(debug_buf);
        ...
        if (http_code == 207) { // Multi-Status，PROPFIND成功
            ...
            while (current_pos != NULL && file_count < 128) {
                ...
                if (href_len < sizeof(filename)) {
                    ...
                    // 检查是否是目标用户的ZIP文件
                    // 需要处理URL编码的用户名（空格变为%20）
                    int is_match = 0;
                    if (strstr(filename, username) && strstr(filename, ".zip")) {
                        is_match = 1;
                    } else {
                        // 如果直接匹配失败，尝试匹配URL编码的用户名
                        char encoded_username[128];
                        const char *src = username;
                        char *dst = encoded_username;
                        while (*src && dst < encoded_username + sizeof(encoded_username) - 3) {
                            if (*src == ' ') {
                                *dst++ = '%';
                                *dst++ = '2';
                                *dst++ = '0';
                        ...
                        *dst = '\0';
                        ...
                        if (strstr(filename, encoded_username) && strstr(filename, ".zip")) {
                            is_match = 1;
                        }
                    }
                    
                    if (is_match) {
                        snprintf(debug_buf, sizeof(debug_buf), "File matches user %s and .zip extension", username);
                        log_file_write(debug_buf);
                        // 提取纯文件名（不含路径）
            ...
            snprintf(debug_buf, sizeof(debug_buf), "Found %d WebDAV backup files for user %s", file_count, username);
            log_file_write(debug_buf);
            ...
            if (file_count >= g_maxback) {
                ...
                for (int i = file_count - 1; i >= file_count - files_to_delete && i >= 0; i--) {
                    ...
                    if (webdav_config.basepath[0] != '\0') {
                        if (webdav_config.origin[strlen(webdav_config.origin)-1] == '/' && webdav_config.basepath[0] == '/') {
                            snprintf(delete_url, sizeof(delete_url), "%s%sJKSV/%s/%s", 
                                     webdav_config.origin, webdav_config.basepath + 1, encoded_game_folder, file_list[i]);
                        } else if (webdav_config.origin[strlen(webdav_config.origin)-1] != '/' && webdav_config.basepath[0] != '/') {
                            snprintf(delete_url, sizeof(delete_url), "%s/%s/JKSV/%s/%s", 
                                     webdav_config.origin, webdav_config.basepath, encoded_game_folder, file_list[i]);
                        } else {
                            snprintf(delete_url, sizeof(delete_url), "%s%sJKSV/%s/%s", 
                                     webdav_config.origin, webdav_config.basepath, encoded_game_folder, file_list[i]);
                        }
                        ...
                        if (webdav_config.origin[strlen(webdav_config.origin)-1] == '/') {
                            snprintf(delete_url, sizeof(delete_url), "%sJKSV/%s/%s", webdav_config.origin, encoded_game_folder, file_list[i]);
                        } else {
                            snprintf(delete_url, sizeof(delete_url), "%s/JKSV/%s/%s", webdav_config.origin, encoded_game_folder, file_list[i]);
                        }
                    }
...
static void manage_backup_count(const char* username, const char* game_folder) {
    ...
    char search_pattern[FS_MAX_PATH];
    snprintf(search_pattern, sizeof(search_pattern), "/autoback/%s/%s", username, game_folder);
