...
// 在主文件中包含 tesla.hpp 之前定义此宏。如果您打算
// 在多个源文件中使用 tesla.hpp 头文件，只需定义一次！
...
namespace tsl {
    ...
    namespace cfg {
        ...
        constexpr u32 ScreenWidth = 1920;       ///< 屏幕宽度
        constexpr u32 ScreenHeight = 1080;      ///< 屏幕高度

        extern u16 LayerWidth;                  ///< Tesla层宽度
        extern u16 LayerHeight;                 ///< Tesla层高度
        extern u16 LayerPosX;                   ///< Tesla层X位置
        extern u16 LayerPosY;                   ///< Tesla层Y位置
        extern u16 FramebufferWidth;            ///< 帧缓冲区宽度
        extern u16 FramebufferHeight;           ///< 帧缓冲区高度
        extern u64 launchCombo;                 ///< 覆盖层激活按键组合
    ...
    namespace style {
        constexpr u32 ListItemDefaultHeight         = 70;       ///< 标准列表项高度
        constexpr u32 TrackBarDefaultHeight         = 90;       ///< 标准轨道条高度
        constexpr u8  ListItemHighlightSaturation   = 6;        ///< 列表项高亮的最大饱和度
        constexpr u8  ListItemHighlightLength       = 22;       ///< 列表项高亮的最大长度

        namespace color {
            constexpr Color ColorFrameBackground  = { 0x0, 0x0, 0x0, 0xD };   ///< 覆盖层框架背景颜色
            constexpr Color ColorTransparent      = { 0x0, 0x0, 0x0, 0x0 };   ///< 透明颜色
            constexpr Color ColorHighlight        = { 0x0, 0xF, 0xD, 0xF };   ///< 绿色高亮颜色
            constexpr Color ColorFrame            = { 0x7, 0x7, 0x7, 0xF };   ///< 外边框颜色
            constexpr Color ColorHandle           = { 0x5, 0x5, 0x5, 0xF };   ///< 轨道条手柄颜色
            constexpr Color ColorText             = { 0xF, 0xF, 0xF, 0xF };   ///< 标准文本颜色
            constexpr Color ColorDescription      = { 0xA, 0xA, 0xA, 0xF };   ///< 描述文本颜色
            constexpr Color ColorHeaderBar        = { 0xC, 0xC, 0xC, 0xF };   ///< 分类标题矩形颜色
            constexpr Color ColorClickAnimation   = { 0x0, 0x2, 0x2, 0xF };   ///< 元素点击动画颜色
        }
    }
    ...
    namespace impl {
        ...
        enum class LaunchFlags : u8 {
            None = 0,                       ///< 启动时不做特殊处理
            CloseOnExit        = BIT(0)     ///< 当最后一个GUI从堆栈弹出时关闭覆盖层
        };
